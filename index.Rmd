...
<br><center><div style="width:50%;"></p>
```{r image_neighborhoods}
knitr::include_graphics(file.path(figs_dir, 'moving-window.png'))
```
<p style="text-align: justify; font-size:90%; width:100%;">
__Figure 9. Image neighborhoods defined as discrete blocks (black lines). In the case of block subsampling, the landscape is subdivided into overlapping 'sublandscapes' (note the shift that occurs as you go from neighborhoods inside of the black lines to those inside of the dashed red lines). The latter approach permits creating neighborhood-based metrics at somewhat higher resolutions (in this case 2X the resolution that would have been obtained by taking the simple, discrete-block approach).__</p></div></center><br>
